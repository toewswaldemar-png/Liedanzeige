...
The server (`server/hub.go`) maintains a `Hub` with four named channels: `lied`, `chor`, `steuerung`, `kiosk`.
...
`Liedanzeige` has two modes: **clock mode** (no input) and **number mode** (hymn number entered). Display settings are applied as CSS custom properties on `<html>` (`--time-large-font-size`, `--gap-time-date`, `--font-family`, `--text-shadow`, etc.). An auto-reset timer returns to clock mode after `resetDelay` minutes.

### Key types (`frontend/src/lib/types.ts`)

`WsMessage` is the discriminated union of all WebSocket message shapes. `DisplaySettings` and `DEFAULTS` define the display configuration shared between `Steuerung` and `Liedanzeige`. **`DEFAULTS` in `types.ts` must stay in sync with the defaults in `server/config.go`.**
...
`server/config.json` holds server/kiosk/screen config and is separate from `settings.json`.

### Kiosk (Wails)

`kiosk/app.go` — on `domReady` (guarded by `sync.Once`), polls `/health` until the server responds, then navigates the Wails window to its screen URL from `config.json`. Screen 0 spawns windows for screens 1+ as subprocesses (production, `--screen N` flag) or browser tabs (dev mode). The app connects to `/ws/kiosk` to receive window-control commands.

`kiosk/monitor.go` (Windows-only) uses Win32 APIs to move windows fullscreen to a specific monitor. `kiosk/numpad.go` (Windows-only, choir screen only) installs a low-level keyboard hook (`WH_KEYBOARD_LL`) that captures numpad input globally — even when the window is unfocused — and forwards digits to `/ws/steuerung`. When the kiosk needs to fade out during a monitor move, it calls `window.__kioskBlackout(bool)` on the Wails webview.

### Watchdog

`watchdog/main.go` monitors `kiosk.exe`, restarting it after a 3s delay on crash. It also subscribes to `/ws/kiosk` and restarts the kiosk on a `"reload"` command. Reads `config.json` from its own directory or parent.
